...
//                @Query("query") query: String?): Observable<MovieListingModel>
    ...
    @GET("discover/movie")
    suspend fun getList(@Query("page") page: String): Response<MovieListingModel>

    @GET("search/movie")
    suspend fun getSearchList(@Query("page") page: String,
                      @Query("query") query: String?): Response<MovieListingModel>
